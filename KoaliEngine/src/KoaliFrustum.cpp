...
void KoaliFrustum::Construct(LPDIRECT3DDEVICE9 pd3dDevice)
{
...
// 点监测
BOOL KoaliFrustum::CheckPoint(D3DXVECTOR3 ptPos)
{
...
// 立方体检测,只要有一个顶点在裁减体之内就认为可见
BOOL KoaliFrustum::CheckCube(D3DXVECTOR3 centerPos, float size)
{
	// 定义立方体的八个顶点
...
// 只要有一个顶点在裁减体之内就认为可见
BOOL KoaliFrustum::CheckRectangle(D3DXVECTOR3 centerPos, float size)
{
	float sizeDivd2 = size / 2;
...
// 球体检测
BOOL KoaliFrustum::CheckSphere(D3DXVECTOR3 centerPos, float radius)
{
	for (int i = 0; i < 6; i++)
	{
