...
CZFXLODTerrain::~CZFXLODTerrain()
{
	...
	d3d::Delete<CZFXLODTrnNode*>(m_pHeadNode);//调用delete pNode会在pNode的析构函数中先释放其四个子节点
...
void CZFXLODTerrain::SetDimesion( float width, float height/*地形的几何尺寸*/,D3DXVECTOR2 centerPt/*地形的中点位置*/ )
{
	m_fWidth = width;
	m_fHeight = height;
	m_centerPt = centerPt;
